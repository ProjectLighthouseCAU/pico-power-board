...
To fix this issue, we need to terminate the transmission line (cable) with properly sized terminating resistors arranged by in a suitable termination technique.  
...
For the transmitting data lines (SCK, MOSI, CS, RESET), we chose series termination since it is the only termination that is recommended at the source. It also is the most widely used technique, gives good results and has no extra power usage. The only downside is that the resistor limits the current and therefore increases the rise-/fall-times of the signal.
...
The Pico has a builtin temperature sensor but it is not very accurate and also inside of the chip where any heat from the CPU is measured as well. We therefore added a small temperature sensor from Texas Instruments (TMP112A) which also has an I2C interface.
